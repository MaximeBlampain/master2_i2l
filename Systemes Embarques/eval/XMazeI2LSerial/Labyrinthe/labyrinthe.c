...
#define DEPLACE_ANGLE		1
#define DEPLACE_X		(MUR_TAILLE/50)
#define DEPLACE_Z		(MUR_TAILLE/50)
...
#define SERIE_PERIPH		"/dev/ttyUSB0"
#define SERIE_VITESSE		9600
...
srand(time(NULL));
point cible={(long)rand()*LABY_X*MUR_TAILLE/RAND_MAX,0,(long)rand()*LABY_Y*MUR_TAILLE/RAND_MAX};
int angle=0;
int touches=0;
int manette=0;
unsigned char fenetre,quitter;
int serie=serialOpen(SERIE_PERIPH,SERIAL_BOTH,0);
if(serie<0){ fprintf(stderr,"Problème série avec '%s' !\n",SERIE_PERIPH); exit(-1); }
serialConfig(serie,SERIE_VITESSE);
while(1){
  ...
  if(touches==0){
    ...
    int nc;
    if(write(serie,"?",1)==1)
      while((nc=read(serie,&c,1))==1){
        if(c=='Q') manette |= TOUCHE_GAUCHE;
        if(c=='D') manette |= TOUCHE_DROITE;
        if(c=='Z') manette |= TOUCHE_HAUT;
        if(c=='S') manette |= TOUCHE_BAS;
        if(c=='*') manette=0;
        }
    if(manette){
      point d=soustraire_points(p,cible);
      int m=abs(d.x)+abs(d.z);
      unsigned char l=m*10/(MUR_TAILLE*LABY_X+MUR_TAILLE*LABY_Y);
      write(serie,"D",1);
      write(serie,&l,1);
      if(l==0){ 
        printf("Cible trouvée en (%d,%d)\n",cible.x/MUR_TAILLE,cible.z/MUR_TAILLE);
        break;
        }
      }
    }
  if(touches || manette){
    if((touches|manette)&TOUCHE_DROITE) angle += DEPLACE_ANGLE;
    if((touches|manette)&TOUCHE_GAUCHE) angle -= DEPLACE_ANGLE;
    if(angle<0 || angle>360) angle=angle%360;
    if((touches|manette)&TOUCHE_HAUT){
      p.x += DEPLACE_X*sin(2*M_PI*angle/360);
      p.z += DEPLACE_Z*cos(2*M_PI*angle/360);
      }
    if((touches|manette)&TOUCHE_BAS){
      p.x -= DEPLACE_X*sin(2*M_PI*angle/360);
      p.z -= DEPLACE_Z*cos(2*M_PI*angle/360);
      }
    }
  if(quitter==1) break;
  if(touches || manette || fenetre){
