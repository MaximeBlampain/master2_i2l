...
unsigned char attendreEvenement(int *touches,unsigned char *fenetre,unsigned char *quitter){
SDL_Event event;
*fenetre=0;
*quitter=0;
unsigned char evenement=0;
while(SDL_PollEvent(&event)!=0){
  evenement=1;
  switch(event.type){
    case SDL_KEYDOWN:
      switch(event.key.keysym.sym){
        case SDLK_LEFT:  *touches |= TOUCHE_GAUCHE; break;
        case SDLK_RIGHT: *touches |= TOUCHE_DROITE; break;
        case SDLK_UP:    *touches |= TOUCHE_HAUT;   break;
        case SDLK_DOWN:  *touches |= TOUCHE_BAS;    break;
        case SDLK_SPACE: *touches |= TOUCHE_ESPACE; break;
        }
      break;
    case SDL_KEYUP:
      switch(event.key.keysym.sym){
